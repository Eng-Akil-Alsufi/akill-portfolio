              ...
              <div className={`h-full bg-card border border-border rounded-2xl overflow-hidden hover:border-primary/50 transition-all duration-500 hover:shadow-2xl hover:shadow-primary/20 hover:-translate-y-3 flex flex-col ${isRtl ? 'text-right' : 'text-left'} relative hover-lift-card card-tilt group/card`}>
                ...
                <div className={`px-6 py-4 border-t border-border/50 flex gap-3 ${isRtl ? 'flex-row-reverse' : ''} bg-card`}>
                  ...
                  <a
                    ...
                    rel="noopener noreferrer"
                    className={`flex-1 ${!project.homepage ? "cursor-not-allowed" : ""}`}
                    onClick={(e) => !project.homepage && e.preventDefault()}
                  >
                    <Button
                      size="sm"
                      disabled={!project.homepage}
                      className={`w-full gap-2 group/btn transition-all duration-300 rounded-xl ripple-effect ${
                        project.homepage 
                          ? "bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20" 
                          : "bg-muted text-muted-foreground opacity-50 border-dashed border-2 border-muted-foreground/30"
                      }`}
                    >
                      {isRtl ? 'معاينة مباشرة' : 'Live Preview'}
                      <ExternalLink className={`w-4 h-4 transition-all ${
                        project.homepage 
                          ? "opacity-0 group-hover/btn:opacity-100 group-hover/btn:translate-x-1" 
                          : "opacity-30"
                      }`} />
                    </Button>
                  </a>
